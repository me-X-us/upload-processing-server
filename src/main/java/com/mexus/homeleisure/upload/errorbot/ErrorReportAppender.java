package com.mexus.homeleisure.upload.errorbot;

import ch.qos.logback.classic.spi.ILoggingEvent;
import ch.qos.logback.core.UnsynchronizedAppenderBase;
import com.mexus.homeleisure.upload.errorbot.config.LogConfig;
import com.mexus.homeleisure.upload.errorbot.util.JsonUtils;
import com.mexus.homeleisure.upload.errorbot.util.MDCUtil;
...
public class ErrorReportAppender extends UnsynchronizedAppenderBase<ILoggingEvent> {
    ...
    protected void append(ILoggingEvent eventObject) {
        if (eventObject.getLevel().isGreaterOrEqual(logConfig.getLevel())) {
            ErrorLogs errorLog = new ErrorLogs(
                    ...
                    MDCUtil.getFromMDC(MDCUtil.REQUEST_URI_MDC),
                    MDCUtil.getFromMDC(MDCUtil.PARAMETER_MAP_MDC),
                    MDCUtil.getFromMDC(MDCUtil.HEADER_MAP_MDC),
                    MDCUtil.getFromMDC(MDCUtil.BODY_MDC),
                    MDCUtil.getFromMDC(MDCUtil.AGENT_DETAIL_MDC)
            );
    ...
    private void sendSlackMessage(ErrorLogs errorLog) {
        ...
        fields.add(generateSlackField(
                "Request Parameter",
                JsonUtils.toPrettyJson(errorLog.getRequestInfo().getParameterMap()),
                true));

        fields.add(generateSlackField(
                "Request Header",
                JsonUtils.toPrettyJson(errorLog.getRequestInfo().getHeaderMap()),
                false));

        fields.add(generateSlackField(
                "Request Body",
                unescapeJava(JsonUtils.toPrettyJson(errorLog.getRequestInfo().getBody())),
                false));
        ...
        fields.add(generateSlackField(
                "User Agent",
                JsonUtils.toPrettyJson(errorLog.getRequestInfo().getAgentDetail()),
                true));
